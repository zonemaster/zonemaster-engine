...
sub all {
    ...
    push @results, $class->address01( $zone ) if Zonemaster::Engine->profile->should_run( 'address01' );
    push @results, $class->address02( $zone ) if Zonemaster::Engine->profile->should_run( 'address02' );
    # Perform ADDRESS03 if ADDRESS02 passed
    if ( any { $_->tag eq q{NAMESERVERS_IP_WITH_REVERSE} } @results ) {
        push @results, $class->address03( $zone ) if Zonemaster::Engine->profile->should_run( 'address03' );
    }
