...
sub basic02 {
    ...
    foreach my $ns ( @ns ) {
        ...
        elsif ( Zonemaster::Engine->profile->ipv4_ok and $ns->address->version == $IP_VERSION_4 ) {
            push @results,
              info(
                IPV4_ENABLED => {
                    ns      => $ns->name->string,
        ...
        elsif ( Zonemaster::Engine->profile->ipv6_ok and $ns->address->version == $IP_VERSION_6 ) {
            push @results,
              info(
                IPV6_ENABLED => {
                    ns      => $ns->name->string,
