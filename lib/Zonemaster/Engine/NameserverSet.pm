...
Adds an item to the set. Objects of type L<Zonemaster::Engine::DNSName> or
L<Zonemaster::Engine::Nameserver> are accepted. Other types are converted to
strings, then turned into L<Zonemaster::Engine::DNSName> objects before being
added to the set.
...
If C<$name_or_ns> is a L<Zonemaster::Engine::DNSName> (or automatically
converted to an object of that type), it is added to the set if and only if
the set contains no L<Zonemaster::Engine::DNSName> object with the same name
nor any L<Zonemaster::Engine::Nameserver> objects with the same name.
...
Returns true if and only if the set contains no items.
