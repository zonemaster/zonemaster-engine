...
=item * Inspect every every message tagged as "fuzzy". Update F<msgstr> if needed
and remove the "fuzzy" mark.

=item * Search for messages that have empty F<msgstr> and add a translation. At
the end of the file there could be lines starting with "#~" and those could have
matching translations.

=item * Any remaining lines at the end of the file starting with "#~" could be
removed.
