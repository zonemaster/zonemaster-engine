...
to keep the database structur of the file.

There are tools that makes editing of the ".po" files easier. When using
those, the ".po" file is handled rather as a database then a plain file.

There is an add-on to Emacs, L<https://www.emacswiki.org/emacs/PoMode>,
which makes updating and searching in the ".po" file easier and more robust.

F<This section needs expansion with more details.>
